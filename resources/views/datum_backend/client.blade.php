             ...
             <div class="card card-block">
                <div class="card-header d-flex justify-content-between pb-0">
                   <div class="header-title">
                    ...
                    <div class="card-title mb-0 d-flex">
                       <h4>{{ optional($user->project)->name }} Housing Project </h4> <br>
                      
                     </div>
                     <h4> Status: 
                    @if(optional($user->project)->status == "done")
                           <span class="mt-2 badge badge-pill badge-success">{{optional($user->project)->status}}</span>
                    @elseif (optional($user->project)->status == "cancelled")
                        <span class="mt-2 badge badge-pill badge-warning">{{optional($user->project)->status}}</span>
                    @elseif (optional($user->project)->status == "delayed")
                       <span class="mt-2 badge badge-pill badge-danger">{{optional($user->project)->status}}</span>
                    @elseif (optional($user->project)->status == "pending")
                       <span class="mt-2 badge badge-pill badge-secondary">{{optional($user->project)->status}}</span>
                    @else
                    {{-- In progress --}}
                      <span class="mt-2 badge badge-pill badge-primary">{{optional($user->project)->status}}</span>
                     @endif
                ...
                <div class="card-body">
                 <h5 class="mb-2 mt-4">Description</h5>
                   <p>{{optional($user->project)->description}}</p>
                   <h5 class="mb-2 mt-4">Start Date</h5>
                   <p>{{optional($user->project)->start_date}}</p>
                   <h5 class="mb-2 mt-4">Due Date</h5>
                   <p>{{optional($user->project)->end_date}}</p>
                   <h5 class="mb-2 mt-4">Alloted Fund</h5>
                   <p>${{optional($user->project)->budget}}</p>
                        ...
                        <div class="card-body">
                           <div class="iq-timeline m-0 d-flex align-items-center justify-content-between position-relative">
                              <ul class="list-inline p-0 m-0 w-100">
                                 ...
                                 @endphp
                                 <li>
                                    <div class="time {{check($counter)}}"> <span>{{$formattedDate}}</span>
        
                                    </div>
                                    <div class="content">
                                       <div class="timeline-dots new-timeline-dots"></div>
                                       <h6 class="mb-1">{{$update->update_title}}</h6>
